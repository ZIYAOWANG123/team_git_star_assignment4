...
keep_md: true    
=======
author: "ZIYAO WANG (Billy)", "Karan Garg", "Jiaying Zhang"
date: "2021/5/15"
output:
...
```
```{r vis3, fig.width=9,fig.height=12,fig.cap="Incidents in each Offence_subgroup of most recorded Offence_subdivision"}
  criminalfinal3<- criminaluse2 %>% 
     mutate(lgth = str_length(`Offence Subdivision`),
            lgth1 = str_length(`Offence Subgroup`)) %>% 
  mutate(`Offence Subdivision` = substr(`Offence Subdivision`,start = 5,stop = lgth),
         `Offence Subgroup` = substr(`Offence Subgroup`,start = 5,stop = lgth1)) %>% 
  group_by(`Local Government Area`, `Offence Subgroup`) %>% 
    summarise(incidents = sum(`Incidents Recorded`)) %>% 
ggplot(aes(y=reorder(`Local Government Area`,incidents),
       x=incidents,
       fill = `Offence Subgroup` )) +
  geom_col() +
  xlab("Distribution of no. of incidents") +
  ylab("Local Government Area")
