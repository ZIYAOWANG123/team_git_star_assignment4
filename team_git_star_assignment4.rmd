...
## What are the top 10 Offences for the most common Suburb found in Q1?

```{r Q3,echo=FALSE,fig.width=8,fig.height=12,,fig.cap="Top 10 Offfences Suburrb wise"}
Suburb_imp <- data3 %>% 
  group_by(Year,Suburb) %>%
  summarise(Total_Incidents = sum(Incidents_Recorded)) %>% 
  arrange(Year,desc(Total_Incidents)) %>% 
...
Suburb1 <- unique(Suburb_imp$Suburb)
...
data3 %>% 
  ...
  ggplot(aes(x =Tot_incidents,
             y= offence1,,
             fill = Offence_Subdivision)) +
  geom_col() +
  geom_text(aes(label = Tot_incidents)) +
  scale_y_reordered() +
  ylab("Type of Offfence") +
  xlab("No of Incidennts") +
  ggtitle("Top 10 Offence for the most common Suburb with incidents found in each year") +
  facet_wrap(~Suburb,ncol = 2,scales = "free")
